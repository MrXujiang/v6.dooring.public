...
import BaseForm from '@/components/BasicShop/ComponentLib/Form/BaseForm'
import BasePopoverForm from '@/components/BasicShop/ComponentLib/Form/BasePopoverForm'
...
const FormItems = (props: FormItemsProps) => {
  ...
  const handleAddItem = (item: baseFormUnion) => {
    const tpl = formTpl.find(v => v.type === item.type)
    const newData = [...formData, { ...tpl!, id: uuid(6, 10) }]
    setFormData(newData)
  ...
  const handleDelItem = (item: baseFormUnion) => {
    const newData = formData.filter(v => v.id !== item.id)
    setFormData(newData)
    onChange && onChange(newData)
  }

  const handleSaveItem = (data: baseFormUnion) => {
    const newData = formData.map(v => (v.id === data.id ? data : v))
    setFormData(newData)
    onChange && onChange(newData)
    setVisible(false)
  }
            ...
            content={
              <>
                <div className={styles.formTpl} style={{ color: 'red' }}>
                  {formTpl.map((item, i) => {
                    const FormItem = BasePopoverForm[item.type]
                    return (
